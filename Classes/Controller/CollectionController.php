...
class CollectionController extends \TQ\TqSlideshow\Controller\AbstractController {
	 ...
	 * @var type
	 */
	protected $_ajaxController	= 'collection';
    ...
    protected function _moduleConf() {
        ...
        // Create
        $params='edit[tx_tq_slideshow_collection]['.$this->pageId.']=new';
        $returnUrl = 'mod.php?M=web_TqSlideshowCollection&action=windowClose';
        $url = 'alt_doc.php?returnUrl='.rawurlencode($returnUrl).'&'.$params;
        $ret['url.slideshow.create'] = $url;

        // Show
        $params='edit[tx_tq_slideshow_collection][T3_DATABASE_ENTITY_UID]=edit';
        $returnUrl = 'mod.php?M=web_TqSlideshowCollection&action=windowClose';
        $url = 'alt_doc.php?returnUrl='.rawurlencode($returnUrl).'&'.$params;
